...
_(Scripts: `dataset_generator.py` and `data_loader.py`)_
...
_(Script: `structural_stream.py`)_
...
_(Scripts: `decoding.py` and `metrics.py`)_
...
2. **MLP Scorer**: A Deep Neural Network (MLPClassifier with layers 256, 128) trains on these features to output a binary probability: _Does sentence $i$ belong before sentence $j$?_
...
_(Script: `prediction_analyzer.py`)_
