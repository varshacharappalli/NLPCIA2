...
**Split:** 100 documents loaded → 80 train / 20 test
...
**Called from main.py as:** `docs = generate_dataset(seed=42, limit=100)`

`generate_dataset()` is a thin wrapper that calls `load_aan_dataset(seed=42, limit=100)`.
...
2. `split = int(0.8 * 100) = 80`
3. Returns `docs[:80]` as train, `docs[80:]` as test
...
All ordered pairs from all 80 training docs. Label +1 means "A comes before B", -1 means "B comes before A".
...
for enc_name, enc in EMBEDDINGS:
    ...
    sample = tfidf_enc.encode_doc(train_full[0])  # (n_sents, 5000)
    gcn_main = GCNEncoder(input_dim=5000, hidden_dim=128, output_dim=64)
    ...
    fusion_main = GatedFusion(all_sem_tr.shape[1], 64, output_dim=out_dim)
    fusion_main.train_gate(all_sem_tr, all_str_tr, n_iters=30)
...
Initialized with: `input_dim=5000, hidden_dim=128, output_dim=64`
...
**`train_gate(sem_train, struct_train, n_iters=30)`**
...
Trained on all pairwise features from 80 training docs.
...
### Pairwise Ordering Accuracy — Test Set (20 docs)
